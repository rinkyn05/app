// ignore_for_file: use_build_context_synchronously // Ignora la advertencia sobre el uso de BuildContext de manera asíncrona.

import 'package:firebase_storage/firebase_storage.dart'; // Importa Firebase Storage para almacenar imágenes GIF.
...
// Función principal para subir una imagen GIF.
Future<void> uploadGifImage(
  ...
  // Selecciona y sube la imagen GIF.
...
// Función para seleccionar y subir la imagen GIF.
  ...
  if (source != null) {
    ...
    if (pickedFile != null) {
      ...
      try {
        ...
        // Genera un nombre de archivo único para la imagen GIF.
        String uniqueFileName = '${uuid.v4()}.$fileExtension';
        // Sube la imagen GIF a Firebase Storage.
        await FirebaseStorage.instance
            .ref('gif_images/$uniqueFileName')
            .putFile(imageFile);
        // Obtiene la URL de descarga de la imagen GIF recién subida.
        String gifUrl = await FirebaseStorage.instance
            .ref('gif_images/$uniqueFileName')
            .getDownloadURL();
        // Llama a la función proporcionada para actualizar la URL de la imagen GIF en la UI.
        updateGifUrl(gifUrl);
      } catch (e) {
        // Manejar errores durante la subida o recuperación de la URL.
      }
    }
  }
}
