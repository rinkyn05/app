...
class ExercisesAntebrazoScreenVid extends StatefulWidget {
  const ExercisesAntebrazoScreenVid({Key? key}) : super(key: key);

  @override
  State<ExercisesAntebrazoScreenVid> createState() =>
      _ExercisesAntebrazoScreenVidState();
}

class _ExercisesAntebrazoScreenVidState
    extends State<ExercisesAntebrazoScreenVid> {
  ...
  Future<List<Ejercicio>> _fetchExercises() async {
    ...
    List<Ejercicio> filteredExercises = allExercises.where((ejercicio) {
      for (var bodypart in ejercicio.bodyParts) {
        if (bodypart['NombreEng'] == 'Antebrazo' &&
            bodypart['NombreEsp'] == 'Antebrazo') {
          return true;
        }
      }
      return false;
  ...
  Future<void> _selectExercise(Ejercicio ejercicio) async {
    ...
    await prefs.setString('selected_body_part_antebrazo', 'Antebrazo');
    await prefs.setString('selected_exercise_name_antebrazo', ejercicio.nombre);
    await prefs.setString(
        'selected_exercise_details_antebrazo', ejercicio.toJson());

    _exerciseNotifier.selectExercise('Antebrazo');
