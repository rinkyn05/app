...
class AgregarCintura extends StatefulWidget {
  final String email;

  const AgregarCintura({
    required this.email,
    Key? key,
  }) : super(key: key);

  @override
  _AgregarCinturaState createState() => _AgregarCinturaState();
}

class _AgregarCinturaState extends State<AgregarCintura> {
  int currentStep = 2;
      ...
      try {
        ...
        if (user != null) {
          await _firestore.collection('usersmedical').doc(widget.email).update({
            'Circunferencias.Cintura': _cintura,
          });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context)!.translate('cintura')),
        leading: IconButton(
