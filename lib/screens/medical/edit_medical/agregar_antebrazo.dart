...
class AgregarAntebrazo extends StatefulWidget {
  final String email;

  const AgregarAntebrazo({
    required this.email,
    Key? key,
  }) : super(key: key);

  @override
  _AgregarAntebrazoState createState() => _AgregarAntebrazoState();
}

class _AgregarAntebrazoState extends State<AgregarAntebrazo> {
  int currentStep = 4;
      ...
      try {
        ...
        if (user != null) {
          await _firestore.collection('usersmedical').doc(widget.email).update({
            'Circunferencias.Antebrazo': _antebrazo,
          });
