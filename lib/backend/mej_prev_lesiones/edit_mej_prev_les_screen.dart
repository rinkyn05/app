...
import '../../functions/mej_prev_lesiones/update_mej_prev_les_functions.dart';
import '../../functions/role_checker.dart';
import '../../functions/upload_exercise_image.dart';

class EditMejPreLesionesScreen extends StatefulWidget {
  final String mejPreLesionesId;
  final Map<String, dynamic> mejPreLesionesData;

  const EditMejPreLesionesScreen(
      {Key? key, required this.mejPreLesionesId, required this.mejPreLesionesData})
      : super(key: key);

  @override
  EditMejPreLesionesScreenState createState() => EditMejPreLesionesScreenState();
}

class EditMejPreLesionesScreenState extends State<EditMejPreLesionesScreen> {
  ...
  Future<void> _initializeFields() async {
    _nameControllerEsp.text = widget.mejPreLesionesData['NombreEsp'] ?? '';
    _nameControllerEng.text = widget.mejPreLesionesData['NombreEng'] ?? '';
    _contenidoControllerEsp.text = widget.mejPreLesionesData['ContenidoEsp'] ?? '';
    _contenidoControllerEng.text = widget.mejPreLesionesData['ContenidoEng'] ?? '';
    _imageUrl = widget.mejPreLesionesData['URL de la Imagen'] ?? '';
    _videoController.text = widget.mejPreLesionesData['Video'] ?? '';
    _membershipEsp = widget.mejPreLesionesData['MembershipEsp'] ?? 'Gratis';
    _membershipEng = widget.mejPreLesionesData['MembershipEng'] ?? 'Free';
  }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text(AppLocalizations.of(context)!.translate('EditMejPreLesiones')),
          bottom: TabBar(
            tabs: tabs,
            labelStyle:
        ...
        body: TabBarView(
          children: [
            _buildMejPreLesionesForm('Esp'),
            _buildMejPreLesionesForm('Eng'),
          ],
        ),
      ),
    );
  ...
  Widget _buildMejPreLesionesForm(String langKey) {
        ...
        child: Column(
          ...
          children: [
            Padding(
              padding: const EdgeInsets.symmetric(vertical: 8.0),
              child: Text(
                AppLocalizations.of(context)!.translate('mejPreLesiones'),
                style: Theme.of(context).textTheme.titleMedium,
              ),
            ),
            TextFormField(
              ...
              decoration: InputDecoration(
                labelText:
                    '${AppLocalizations.of(context)!.translate('mejPreLesiones')} ($langKey)',
                hintText:
                    AppLocalizations.of(context)!.translate('enterMejPreLesionesName'),
                border: borderStyle,
    ...
    return _buildCustomButton(
      ...
      () async {
        ...
        if (_nameControllerEsp.text.isNotEmpty &&
          ...
          await UpdateMejPreLesionesFunctions().updateMejPreLesiones(
            id: widget.mejPreLesionesId,
  ...
  void _showSuccessSnackbar(BuildContext context) {
    Fluttertoast.showToast(
      msg: AppLocalizations.of(context)!.translate('mejPreLesionesUpdated'),
