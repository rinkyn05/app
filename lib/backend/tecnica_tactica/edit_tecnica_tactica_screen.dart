...
import '../../functions/tecnica_tactica/update_tecnica_tactica_functions.dart';
import '../../functions/upload_exercise_image.dart';

class EditTenicaTacticaScreen extends StatefulWidget {
  final String tenicaTacticaId;
  final Map<String, dynamic> tenicaTacticaData;

  const EditTenicaTacticaScreen(
      {Key? key, required this.tenicaTacticaId, required this.tenicaTacticaData})
      : super(key: key);

  @override
  EditTenicaTacticaScreenState createState() => EditTenicaTacticaScreenState();
}

class EditTenicaTacticaScreenState extends State<EditTenicaTacticaScreen> {
  ...
  Future<void> _initializeFields() async {
    _nameControllerEsp.text = widget.tenicaTacticaData['NombreEsp'] ?? '';
    _nameControllerEng.text = widget.tenicaTacticaData['NombreEng'] ?? '';
    _contenidoControllerEsp.text = widget.tenicaTacticaData['ContenidoEsp'] ?? '';
    _contenidoControllerEng.text = widget.tenicaTacticaData['ContenidoEng'] ?? '';
    _imageUrl = widget.tenicaTacticaData['URL de la Imagen'] ?? '';
    _videoController.text = widget.tenicaTacticaData['Video'] ?? '';
    _membershipEsp = widget.tenicaTacticaData['MembershipEsp'] ?? 'Gratis';
    _membershipEng = widget.tenicaTacticaData['MembershipEng'] ?? 'Free';
  }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text(AppLocalizations.of(context)!.translate('EditTenicaTactica')),
          bottom: TabBar(
            tabs: tabs,
            labelStyle:
        ...
        body: TabBarView(
          children: [
            _buildTenicaTacticaForm('Esp'),
            _buildTenicaTacticaForm('Eng'),
          ],
        ),
      ),
    );
  ...
  Widget _buildTenicaTacticaForm(String langKey) {
        ...
        child: Column(
          ...
          children: [
            Padding(
              padding: const EdgeInsets.symmetric(vertical: 8.0),
              child: Text(
                AppLocalizations.of(context)!.translate('tenicaTactica'),
                style: Theme.of(context).textTheme.titleMedium,
              ),
            ),
            TextFormField(
              ...
              decoration: InputDecoration(
                labelText:
                    '${AppLocalizations.of(context)!.translate('tenicaTactica')} ($langKey)',
                hintText:
                    AppLocalizations.of(context)!.translate('enterTenicaTacticaName'),
                border: borderStyle,
    ...
    return _buildCustomButton(
      ...
      () async {
        ...
        if (_nameControllerEsp.text.isNotEmpty &&
          ...
          await UpdateTenicaTacticaFunctions().updateTenicaTactica(
            id: widget.tenicaTacticaId,
  ...
  void _showSuccessSnackbar(BuildContext context) {
    Fluttertoast.showToast(
      msg: AppLocalizations.of(context)!.translate('tenicaTacticaUpdated'),
